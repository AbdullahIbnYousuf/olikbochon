...
|---|---|---|---|---|---|---|---|
| [`csebuetnlp/banglabert`](https://huggingface.co/csebuetnlp/banglabert) | Bengali-native ELECTRA discriminator; its model card reports 110M parameters and strong Bengali classification/NLI results. | 443 MB checkpoint; about 4–8 GB training and 1–2 GB inference VRAM at length 256, depending on batch/optimizer. | Roughly 10–40 minutes per run; normalization adds preprocessing complexity. | Bengali native. | Use the authenticated private snapshot, pinned normalizer, and later private classification checkpoint; load from a local Kaggle path only. | Model and normalizer declare CC BY-NC-SA 4.0. Private use proceeds under the user's noncommercial student-competition facts; public derived-weight publication remains restricted pending applicable notices and organizer requirements. | Medium; authentication and offline loading now pass, while training/runtime still require validation. |
